   ...
   for step in progress:
      ...
      if step < configs['learning_start']:
         ...
         with torch.no_grad():   
            ...
            noise = np.random.normal(0, configs['exploration_noise']) * action_range
      ...
      if step >= configs['learning_start'] and step % configs['update_every'] == 0:
         for _ in range(configs['update_every']):
            ...
            with torch.no_grad():
               ...
               next_q = qf_target(batch.next_observations, next_action).flatten()
               target = batch.rewards.flatten() + configs['gamma'] * (1 - batch.dones.flatten()) * next_q
            
            current_q = qf(batch.observations, batch.actions).flatten()
            qf_loss = F.mse_loss(current_q, target)
            qf_optimizer.zero_grad()
            qf_loss.backward()
            torch.nn.utils.clip_grad_norm_(qf.parameters(), configs['grad_clip'])
            qf_optimizer.step()
            
            # Atualização da rede pi
            pi_loss = -qf(batch.observations, pi(batch.observations)).mean()
            ...
            if qf_loss.item() < best_qf_loss:
               torch.save(qf.state_dict(), f"models/{run_name}/qf.pth")
